<?php
    ...
    $mysqli = new mysqli("localhost","hostel", "hostel", "test_db");
    ...
    if ($password !== $cpassword) {
	    ...
	    exit(); // stop execution if they don't match
    }

    $password = md5($password); // or better: use password_hash()
    
    $checkQuery = "SELECT regNo FROM userregistration WHERE regNo = ? OR email = ?";
    $checkStmt = $mysqli->prepare($checkQuery);
    $checkStmt->bind_param('ss', $regno, $emailid);
    $checkStmt->execute();
    $checkStmt->store_result();

    if ($checkStmt->num_rows > 0) {
	    echo "<script>alert('User with this Reg No or Email already exists!');</script>";
	    exit();
    }
    ...
    if ($stmt->execute()) 
        {
		 ...
		 //Image upload backend logic
	       if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK ){
			...
			$query = mysqli_query($mysqli, "INSERT INTO girl_profile_image (file, regno) VALUES ('$file_name', '$regno')");
                                ...
                                <div class="card-body">
                                    <h4 class="card-title">Email ID</h4>
                                        <div class="form-group">
                                            ...
                                            <img src="loader.gif" id="loaderIcon" style="display:none" />
                                        </div>
                                </div>
                            </div>
                        </div>
